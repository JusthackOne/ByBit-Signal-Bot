...
import { formatNumberToMillion, update_oi, update_pump, update_rekt } from "../../utils/messages";
import { IByBitApiResponse, IByBitApiResponseLiquidation } from "../api.service";
...
class BinanceService {
  ...
  public async clearSignals(diff) {
    ...
    await ByBitLinear_OI.updateMany({}, { h24_signal_count_growth: 0, h24_signal_count_recession: 0 });
  ...
  async subcribeTicker(symbol: string, type: CategoryV5) {
    try {
      ...
      loggerWithCtx.debug(undefined, `Error to subcribe (start app) to ticker ${symbol} type: ${type}`, error);
      return false;
    }
  }
  ...
  async subcribeLiquidation(symbol: string, type: CategoryV5) {
    try {
      ...
      loggerWithCtx.debug(undefined, `Error to subcribe (start app) to Liquidation ${symbol} type: ${type}`, error);
      return false;
    }
  }
  ...
  async unSubcribeLiquidation(symbol: string, type: CategoryV5): Promise<Boolean> {
    try {
  ...
  public async getTickerUpdateOI(data: IByBitApiResponse): Promise<void> {
    ...
    const tickerData = (
      await axios.get(`https://api.bybit.com/v5/market/tickers?category=linear&symbol=${updateData.symbol}`)
    ).data.result.list[0];
    ...
    const { updateTimeGrowth, updateTimeRecession } = getUpdateTimeOI(ticker, config);
    ...
    const { oi_change_growth, oi_change_recession } = getOpenInterestValueChange(updateData, ticker);

    const change_price: number = Number(
      calculatePercentageChange(ticker.lastPriceGrowth, tickerData.lastPrice).toFixed(2)
    );
    ...
    } else if (oi_change_recession < 0 && Math.abs(oi_change_recession) >= config.oi_recession_percentage) {
      const oi_change_value: number = formatNumberToMillion(
  ...
  public async getTickerUpdatePUMP(data: IByBitApiResponse, type: CategoryV5): Promise<void> {
    const updateData = data.data;
    const tickerData = (
      await axios.get(`https://api.bybit.com/v5/market/tickers?category=${type}&symbol=${updateData.symbol}`)
    ).data.result.list[0];
    ...
    const { updateTimeGrowth, updateTimeRecession } = getUpdateTimePUMP(ticker, config);
    ...
    const { pump_change_recession, pump_change_growth } = getPumpChange(updateData, ticker);
    ...
    } else if (pump_change_recession < 0 && Math.abs(pump_change_recession) >= config.pump_recession_percentage) {
  ...
  public async getTickerUpdateREKT(data: IByBitApiResponseLiquidation): Promise<void> {
    const updateData = data.data;
    ...
    if (Number(updateData.price) >= config.rekt_limit) {
      ...
      for (const id of IDS) {
        try {
          await this.Bot.telegram.sendMessage(
            id,
            update_rekt(updateData.symbol, Number(updateData.price), updateData.side, signals_count),
            {
              parse_mode: "HTML",
              link_preview_options: { is_disabled: true },
            }
